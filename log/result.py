            ...
            for tx_hash in tx_hashes:
                ...
                if block_hash not in written_block_hashes[from_shard][to_shard]:
                    ...
                    with open(output_file, 'a') as file:
                        if block_hash in extracted_logs:
...
def print_time_differences(time_differences, output_file):
    average_times = {}  # 用于保存平均时间
    for from_shard, to_shard_logs in time_differences.items():
        for to_shard, logs in to_shard_logs.items():
            ...
            if count > 0:
                ...
                average_times[f"{from_shard}->{to_shard}"] = round(average_time, 6)
                print(f"从 {from_shard} 到 {to_shard} 的 {count} 笔跨片交易平均交易耗时为: {average_time:.6f} 秒")
            else:
                average_times[f"{from_shard}->{to_shard}"] = None  # 没有有效交易的情况
            # 将结果写入 JSON 文件
    with open(output_file, 'w') as f:
        json.dump(average_times, f, ensure_ascii=False, indent=4)
    print(f"平均时间结果已保存到 {output_file}。")

def print_time_differences2(time_differences):
    for from_shard, to_shard_logs in time_differences.items():
        for to_shard, logs in to_shard_logs.items():
            print(f"从 {from_shard} 到 {to_shard} 的交易耗时为：")
            total_time = 0
...
def print_time_differences3(time_differences):
    for from_shard, to_shard_logs in time_differences.items():
        for to_shard, logs in to_shard_logs.items():
            print(f"从 {from_shard} 到 {to_shard} 的交易耗时为：")
            total_time = 0
            count = 0
            max_transactions = 10  # 限制最大交易数量
            
            for log in logs:
                if count >= max_transactions:  # 超过限制时停止
                    break
            ...
            if count > 0:
                average_time = total_time / count
                print(f"从 {from_shard} 到 {to_shard} 的前 {count} 笔跨片交易平均交易耗时为: {average_time:.6f} 秒")
            else:
                print(f"从 {from_shard} 到 {to_shard} 没有交易记录")
            print()
...
def main():
    ...
    print_time_differences(time_differences,output_file)
